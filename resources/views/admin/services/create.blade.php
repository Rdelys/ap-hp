...
@section('contenu')
    ...
    <h1 class="font-titre text-2xl mt-4 mb-8 pb-5 border-b border-ligne">Ajouter un service</h1>
    ...
    <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-5 max-w-lg">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="text" name="referent_nom" value="{{ old('referent_nom') }}" class="w-full border border-ligne px-3 py-2.5 text-[14px]">
        </div>
        <div>
            <label class="block text-[12px] text-meta mb-1.5">Référent — email</label>
            <input type="email" name="referent_email" value="{{ old('referent_email') }}" class="w-full border border-ligne px-3 py-2.5 text-[14px]">
        </div>
        <div>
            <label class="block text-[12px] text-meta mb-1.5">Référent — téléphone</label>
            <input type="text" name="referent_telephone" value="{{ old('referent_telephone') }}" class="w-full border border-ligne px-3 py-2.5 text-[14px]">
        </div>
        <div>
            <label class="block text-[12px] text-meta mb-1.5">Règle de nommage des documents</label>
            <input type="text" name="regle_nommage" value="{{ old('regle_nommage') }}" placeholder="ex: {dictant}_{reference}_{date}"
                   class="w-full border border-ligne px-3 py-2.5 text-[14px] font-mono">
            <p class="text-[11px] text-meta mt-1">Variables disponibles : {reference}, {demandeur}, {dictant}, {date}, {type}</p>
        </div>
        <label class="flex items-center gap-2 text-[13px]">
            <input type="checkbox" name="ia_autorisee" value="1" checked class="accent-black">
